/*
  FlowForge Manager — Interface Rename View (Debian)
...
(() => {
  ...
  window.renderRenameView = function renderRenameView() {
    ...
    const serverOptions = servers.map(s => `<option value="${ffEscapeAttr(s.name)}">${ffEscapeHtml(s.name)}</option>`).join("");

    const defaultUdevPath = (window.CFG.debian && window.CFG.debian.udevPath)
      ? window.CFG.debian.udevPath
      : "/etc/udev/rules.d/10-flowforge-ifnames.rules";
          ...
          <div class="bd">
            <div class="two" style="margin-bottom:10px">
              ...
              <div>
                ...
                <input id="renameScriptName" value="flowforge-ifrename.sh" />
              </div>
            </div>
            ...
            <div class="two" style="margin-bottom:10px">
              <div>
                ...
                <input id="renameUdevPath" value="${ffEscapeAttr(defaultUdevPath)}" />
              </div>
              <div>
  ...
  function bindRenameEvents() {
    $("#renameBuildBtn").on("click", () => {
      ...
      if (!serverName) {
        setStatus("#renameStatus", "Please select a server.", "warn");
        return;
      }

      window.CFG.debian = window.CFG.debian || {};
      window.CFG.debian.udevPath = $("#renameUdevPath").val().trim() || "/etc/udev/rules.d/10-flowforge-ifnames.rules";
      window.CFG.debian.disablePredictable = $("#renameDisablePredictable").val() === "true";
      window.saveConfig();

      const block = buildCopyBlock(serverName);
      $("#renameOut").val(block);
      setStatus("#renameStatus", "Copy block generated.", "ok");
    });

    $("#renameCopyBtn").on("click", async () => {
      const txt = $("#renameOut").val();
      if (!txt) {
        setStatus("#renameStatus", "No output available.", "warn");
        return;
      }
      try {
        await navigator.clipboard.writeText(txt);
        setStatus("#renameStatus", "Copied.", "ok");
      } catch {
        setStatus("#renameStatus", "Copy not available (browser permissions).", "bad");
      }
    });
  }
  ...
  function buildCopyBlock(serverName) {
    ...
    const scriptName = $("#renameScriptName").val().trim() || "flowforge-ifrename.sh";

    const copy = [];
    copy.push("# FlowForge Manager — Debian Interface Rename");
...
if [ \${#IFROWS[@]} -eq 0 ]; then
  echo "[FlowForge] ERROR: No global IPv4 addresses found." >&2
  exit 2
fi

# Base rules header
cat > "$TMP_RULES" <<'HDR'
# Generated by FlowForge Manager
...
echo "[FlowForge] Done. Reboot is recommended."
echo "[FlowForge] Suggested: sudo reboot"
`);
    copy.push("EOF");
